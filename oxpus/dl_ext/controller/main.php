	...
	public function handle($view = '')
	{
		...
		*/
		...
		if ($view != 'bug_tracker')
		{
			...
			else
			{
				...
				{
					trigger_error($this->user->lang['DL_OFF_MESSAGE']);
				}
		...
		*/
		...
		if ($view != 'hacks')
		{
			...
			$nav_string['name'][] = $this->user->lang['DL_CAT_TITLE'];
		}
		...
		switch ($view)
		{
			...
			case 'thumbs':
				...
				$nav_string['name'][] = $this->user->lang['DL_EDIT_THUMBS'];
			break;
			case 'comment':
				$nav_string['link'][] = array('view' => 'detail', 'df_id' => $df_id);
				$nav_string['name'][] = $this->user->lang['DL_DETAIL'] . ': ' . $description;
			...
			case 'upload':
				...
				$nav_string['name'][] = $this->user->lang['DL_UPLOAD'];
			break;
			case 'modcp':
				if ($action == 'edit')
				{
					...
					$nav_string['name'][] = $this->user->lang['DL_EDIT_FILE'];
				}

				if ($action == 'approve')
				{
					$nav_string['link'][] = array('view' => 'modcp', 'action' => 'approve');
					$nav_string['name'][] = $this->user->lang['MCP'] . ' <strong>&#8249;</strong> ' . $this->user->lang['DOWNLOADS'] . ' ' . $this->user->lang['DL_APPROVE'];
				}
				else if ($action == 'capprove')
				{
					$nav_string['link'][] = array('view' => 'modcp', 'action' => 'capprove');
					$nav_string['name'][] = $this->user->lang['MCP'] . ' <strong>&#8249;</strong> ' . $this->user->lang['DL_APPROVE_COMMENTS'];
				}
				else if ($action != 'edit')
				{
					if (!$cat_id)
					...
					$nav_string['name'][] = $this->user->lang['MCP'];
				}
			break;
			case 'bug_tracker':
				$nav_string['link'][] = array('view' => 'bug_tracker', 'df_id' => $df_id);
				$nav_string['name'][] = $this->user->lang['DL_BUG_TRACKER'];
			break;
			case 'stat':
				$nav_string['link'][] = array('view' => 'stat');
				$nav_string['name'][] = $this->user->lang['DL_STATS'];
			break;
			case 'user_config':
				$nav_string['link'][] = array('view' => 'user_config');
				$nav_string['name'][] = $this->user->lang['DL_CONFIG'];
			break;
			case 'search':
				$nav_string['link'][] = array('view' => 'search');
				$nav_string['name'][] = $this->user->lang['SEARCH'];
			break;
			case 'hacks':
				$nav_string['link'][] = array('view' => 'hacks');
				$nav_string['name'][] = $this->user->lang['DL_HACKS_LIST'];
			break;
			case 'todo':
				$nav_string['link'][] = array('view' => 'todo');
				$nav_string['name'][] = $this->user->lang['DL_MOD_TODO'];
		...
		*/
		if ($view == 'todo')
		{
			...
			{
				trigger_error($this->user->lang['DL_NO_PERMISSION'], E_USER_WARNING);
			}
			...
			else
			{
				...
				page_header($this->user->lang['DL_MOD_TODO']);
		...
		*/
		if ($view == 'broken' && $df_id && $cat_id && ($this->user->data['is_registered'] || (!$this->user->data['is_registered'] && $this->config['dl_report_broken'])))
		{
			...
			else
			{
				...
				$report_notify_text = ($report_notify_text) ? sprintf($this->user->lang['DL_REPORT_NOTIFY_TEXT'], $report_notify_text) : '';
		...
		*/
		if ($view == 'bug_tracker' && $this->user->data['is_registered'])
		{
			...
			{
				$inc_module = true;
				page_header($this->user->lang['DL_BUG_TRACKER']);
		...
		*/
		...
		if ($view == 'stat')
		{
			...
			*/
			$inc_module = true;
			page_header($this->user->lang['DL_STATS']);
		...
		else if ($view == 'user_config')
		{
			...
			*/
			$inc_module = true;
			page_header($this->user->lang['DL_CONFIG']);
		...
		else if ($view == 'search')
		{
			...
			*/
			$inc_module = true;
			page_header($this->user->lang['SEARCH'].' '.$this->user->lang['DOWNLOADS']);
		...
		{
			$inc_module = true;
			page_header($this->user->lang['DL_UPLOAD']);
